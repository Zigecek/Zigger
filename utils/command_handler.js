...
bot.on("messageCreate", (message) => {
  if (message.channel.type != "DM" && message.author != bot.user) {
    if (message.author != bot.user && message.author.bot == false) {
      var gExist = false;
      Guild.exists({ guildID: message.guild.id }, function (err, res) {
        if (err) {
          console.error(err);
          error.sendError(err);
          return;
              ...
              if (
                ...
                if (map.values().next().value) {
                  ...
                  if (
                    ...
                    Gres.cooldowns[map.values().next().value.name] ==
                      undefined ||
                    ...
                    if (map.values().next().value.category == "music") {
                      if (Gres.blacklist.includes(message.channel.id)) {
                        message.channel.send(
                          LMessages.music.channelBlacklisted
                        );
                        return;
                      }
                    }
...
bot.on("interactionCreate", async (int) => {
  ...
  Guild.exists({ guildID: int.guild.id }, async (err, res) => {
    if (err) {
      console.error(err);
      error.sendError(err);
      return;
